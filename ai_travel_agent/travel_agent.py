...
def search_metaso(query: str) -> str:
    ...
    formatted = []
    for i, page in enumerate(webpages[:10], 1):
        formatted.append(f"{i}. {page.get('title', '')}\n   {page.get('snippet', '')}\n   URL: {page.get('link', '')}")

    return "\n\n".join(formatted)
#model=OpenAIChat(id="glm-4",api_key=zhipu_api_key)
...
st.caption("Plan your next adventure with AI Travel Planner - powered by GLM-4-flash + Metaso Search")
...
zhipu_api_key = st.text_input("Enter Zhipu API Key (glm-4-flash)", type="password")

# Get SerpAPI key from the user
metaso_api_key = st.text_input("Enter Metaso API Key", type="password")

if zhipu_api_key and metaso_api_key:
    researcher = Agent(
        name="Researcher",
        ...
        instructions=[
            ...
            "For each search term, call `search_metaso` and analyze the results.",
    ...
    planner = Agent(
        ...
        model=OpenAIChat(id="glm-4-flash", api_key=zhipu_api_key),
        description=dedent(
            """\
